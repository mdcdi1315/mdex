...
public final class PlayerPlacementInformation
{
    ...
    {
        this();
        Optional<String> os = tag.getString("source_dimension");
        Optional<int[]> oblkpos = tag.getIntArray("teleporter_position");
        Optional<CompoundTag> oct = tag.getCompound("source_dimension_position");
        if (os.isPresent()) {
            source_dimension = ResourceLocation.tryParse(os.get());
        ...
        if (oblkpos.isPresent()) {
            int[] data = oblkpos.get();
