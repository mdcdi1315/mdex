        ...
        extends AbstractMineshaftPiece
{
        ...
        public MineShaftCrossing(int genDepth, BoundingBox boundingBox, @MaybeNull Direction direction, CustomizableMineshaftStructureSettings settings) {
        ...
        private void placeSupportPillar(WorldGenLevel level, BoundingBox box, int x, int y, int z, int maxY) {
            if (BlockUtils.ReferentIsSolidBlock(this.getBlock(level, x, maxY + 1, z, box))) {
                this.generateBox(level, box, x, y, z, x, maxY, z, settings.PlanksState.BlockState, CAVE_AIR, false);
            }
        }
}
