    ...
    extends Feature<BaseTeleporterPlacementFeatureConfiguration>
{
    ...
    public boolean place(FeaturePlaceContext<BaseTeleporterPlacementFeatureConfiguration> fpc)
    {
        ...
        {
            MDEXModInstance.LOGGER.error("Cannot place the feature because it's configuration is invalid.");
            return false;
        }
        ...
        for (BlockPos temp : FeaturePlacementUtils.GetRectangularArea(pos.offset(-size , -1, -size) , new BlockPos(2+size , 4, 2+size)))
        {
            if (temp.getY() == ydownlayer) {
                ...
                {
                    MDEXModInstance.LOGGER.error("MDEXTELEPORTER_EVENTS: Failed to place a plate block at {}." , temp);
                    return false;
                }
        ...
        {
            MDEXModInstance.LOGGER.error("MDEXTELEPORTER_EVENTS: Failed to place the teleporter down.");
            return false;
        }
        ...
        if (wgl.getMaxLocalRawBrightness(pos) < 4)
        {
            ...
            })
            {
                ...
                {
                    MDEXModInstance.LOGGER.error("MDEXTELEPORTER_EVENTS: Failed to place α light block at {}." , temp);
                    return false;
                }
            }
        }
        ...
        if (fpc.config().PlaceStarterChest)
        {
            ...
            if (si.Probability > 0f && rs.nextFloat() < si.Probability) {
                ...
                if (!wgl.setBlock(chestpos , si.ContainerState.GetBlockState(wgl, rs , chestpos) , 2)) {
                    MDEXModInstance.LOGGER.info("MDEXTELEPORTER_EVENTS: Failed to place the starter chest for this dimension.");
                }
                if (!BlockUtils.SetRandomizableContainerLootTable(wgl , rs , chestpos , si.LootTable)) {
                    MDEXModInstance.LOGGER.info("MDEXTELEPORTER_EVENTS: Failed to create the loot table for the starter chest. Removing it.");
