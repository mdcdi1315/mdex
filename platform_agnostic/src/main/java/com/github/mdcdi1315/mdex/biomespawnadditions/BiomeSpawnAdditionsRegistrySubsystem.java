...
import com.github.mdcdi1315.basemodslib.BaseModsLib;  // Provides whether a mod is loaded or not.
...
public final class BiomeSpawnAdditionsRegistrySubsystem
{
    ...
    public static void InitializeRegistry(IRegistryRegistrar registrar)
    {
        ...
        registrar.RegisterDatapackRegistry(REGISTRYKEY, BiomeSpawnAdditions.GetCodec());
    }
    ...
    {
        MDEXModInstance.LOGGER.info("BiomeSpawnAdditions: Applying {} biome spawn addition objects" , spawnadditions.size());
        ...
        for (var entry : spawnadditions)
        {
            ...
            for (String mod : entry.ModIds)
            {
                if (!BaseModsLib.IsModLoaded(mod)) {
                    MDEXModInstance.LOGGER.warn("BiomeSpawnAdditions: Cannot load entry with ID {} because the mod with ID '{}' is not present in the mod lifecycle." , spawnadditions.getKey(entry) , mod);
                    ...
                    if ((biometag = entry.Biomes.left()).isPresent()) {
                        ...
                        for (var i : entry.Biomes.right().get()) {
                            ...
                            if ((t = biomes.getOptional(i)).isPresent()) {
                                ...
                                // Emit a warning.
                                MDEXModInstance.LOGGER.warn("BiomeSpawnAdditions: Cannot find the biome with ID {}. Biome spawn additions will not happen for this biome object.", i);
                            }
                        }
                    }
                }
        ...
        MDEXModInstance.LOGGER.info("BiomeSpawnAdditions: Applied {} biome spawn addition modifications to {} biomes" , applied , appliedbiomes);
    }
