...
import com.github.mdcdi1315.mdex.block.blockstateproviders.BlockStateProviderContext;
...
import com.github.mdcdi1315.mdex.features.config.AdvancedFloatingIslandConfigurationDetails;
...
public final class AdvancedFloatingIslandFeature
    extends ModdedFeature<ModdedFeatureConfiguration<AdvancedFloatingIslandConfigurationDetails>>
{
    public AdvancedFloatingIslandFeature(Codec<ModdedFeatureConfiguration<AdvancedFloatingIslandConfigurationDetails>> codec) {
        super(codec);
    }

    @Override
    protected boolean PlaceModdedFeature(FeaturePlaceContext<ModdedFeatureConfiguration<AdvancedFloatingIslandConfigurationDetails>> fpc)
    {
        AdvancedFloatingIslandConfigurationDetails details = fpc.config().Details;
        ...
        layer = layer.above(details.MaxDistanceFromGround.sample(fpc.random()));

        // Check that we have enough space to generate the feature
        if (!SimpleFloatingIslandFeature.IsConsideredValidPlacement(wgl , layer , details.Layers.size())) { return false; }
        ...
        for (int I = details.Layers.size() - 1; I > -1; I-- , layer = layer.above())
        {
            if (!wgl.ensureCanWrite(layer))
            {
                MDEXModInstance.LOGGER.warn("Cannot place the simple floating island because the level is not writeable here.");
                return false;
            }
            dims = PlaceSingleLayer(wgl , layer , predicate , fpc.random() , details.Layers.get(I));
        }
        ...
        FeaturePlacementUtils.ForEachInHolderSetUnsafe(details.FeaturesToGenerateOnTop , fpc , layer , dims , AdvancedFloatingIslandFeature::RunTopFeature);

        return true;
    }

    private static void RunTopFeature(PlacedFeature p , FeaturePlaceContext<ModdedFeatureConfiguration<AdvancedFloatingIslandConfigurationDetails>> cfg , BlockPos layer , FloatingIslandLayerDimensions lastlayerdims)
    {
        var rd = cfg.random();
    ...
    private static FloatingIslandLayerDimensions PlaceSingleLayer(WorldGenLevel level, BlockPos layerpos , Predicate<BlockState> predicate , RandomSource rs , AdvancedCompilableIslandLayer layer)
    {
        ...
        BlockPos pos1 , pos2;
        var providercontext = new BlockStateProviderContext(level , rs , layerpos);
        // Place the central block first.
        FeaturePlacementUtils.SafeSetBlock(level , layerpos , layer.Provider.GetBlockState(providercontext) , predicate);
        // Modify Z and expand the blocks there.
        for (int Z = 1; Z < wz; Z++)
        {
            pos1 = layerpos.offset(0 , 0 , Z);
            pos2 = layerpos.offset(0 , 0 , -Z);
            FeaturePlacementUtils.SafeSetBlock(level , pos1 , layer.Provider.GetBlockState(providercontext.WithNewPosition(pos1)) , predicate);
            FeaturePlacementUtils.SafeSetBlock(level , pos2 , layer.Provider.GetBlockState(providercontext.WithNewPosition(pos2)) , predicate);
            BlockPos pzx1 , pzx2;
            // Expand the block column by Z_WIDTH / 2.
            for (int ZX = wz / 2; ZX > 0; ZX--)
            {
                pzx1 = pos1.offset(ZX , 0 , 0);
                FeaturePlacementUtils.SafeSetBlock(level , pzx1 , layer.Provider.GetBlockState(providercontext.WithNewPosition(pzx1)) , predicate);
                pzx1 = pos1.offset(-ZX , 0 , 0);
                FeaturePlacementUtils.SafeSetBlock(level , pzx1 , layer.Provider.GetBlockState(providercontext.WithNewPosition(pzx1)) , predicate);
                pzx2 = pos2.offset(ZX , 0 , 0);
                FeaturePlacementUtils.SafeSetBlock(level , pzx2 , layer.Provider.GetBlockState(providercontext.WithNewPosition(pzx2)) , predicate);
                pzx2 = pos2.offset(-ZX , 0 , 0);
                FeaturePlacementUtils.SafeSetBlock(level , pzx2 , layer.Provider.GetBlockState(providercontext.WithNewPosition(pzx2)) , predicate);
            }
        }
        ...
        for (int X = 1; X < wx; X++)
        {
            ...
            for (int XZ = wx / 2; XZ > 0; XZ--)
            {
                ...
                FeaturePlacementUtils.SafeSetBlock(level , pxz1 , layer.Provider.GetBlockState(providercontext.WithNewPosition(pxz1)) , predicate);
                pxz1 = pos1.offset(0 , 0 , -XZ);
                FeaturePlacementUtils.SafeSetBlock(level , pxz1 , layer.Provider.GetBlockState(providercontext.WithNewPosition(pxz1)) , predicate);
                pxz2 = pos2.offset(0 , 0 , XZ);
                FeaturePlacementUtils.SafeSetBlock(level , pxz2 , layer.Provider.GetBlockState(providercontext.WithNewPosition(pxz2)) , predicate);
                pxz2 = pos2.offset(0 , 0 , -XZ);
                FeaturePlacementUtils.SafeSetBlock(level , pxz2 , layer.Provider.GetBlockState(providercontext.WithNewPosition(pxz2)) , predicate);
            }
        }
