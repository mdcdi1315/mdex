...
public final class FeaturePlacementUtils
{
    ...
    public static <TF extends PlacedFeature , TData> void ForEachInHolderSetUnsafe(HolderSet<TF> hs , @MaybeNull TData data , Action2<TF , TData> action)
    {
        for (var feature : hs)
        {
            ...
            {
                MDEXModInstance.LOGGER.warn("FPUtils: Cannot access a feature required to be generated. Check your config and the feature path you have provided.");
                continue;
            }
    ...
    private static void TrySpawnEntityAtChunkGenPhaseInternal(ServerLevelAccessor level , BlockPos finalpos , EntityType<?> type)
    {
        ...
        if (ent instanceof LivingEntity le)
        {
            ...
            MDEXModInstance.LOGGER.warn("Cannot spawn entity of type {} because it does not derive from LivingEntity. Skipping entity generation." , type.getClass().getName());
        }
    }
