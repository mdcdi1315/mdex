...
import com.github.mdcdi1315.mdex.features.config.ModdedOreVeinFeatureConfigurationDetails;
...
public final class ModdedOreVeinFeature
    extends ModdedFeature<ModdedFeatureConfiguration<ModdedOreVeinFeatureConfigurationDetails>>
{
    public ModdedOreVeinFeature(Codec<ModdedFeatureConfiguration<ModdedOreVeinFeatureConfigurationDetails>> codec) {
        super(codec);
    }

    @Override
    protected boolean PlaceModdedFeature(FeaturePlaceContext<ModdedFeatureConfiguration<ModdedOreVeinFeatureConfigurationDetails>> fpc)
    {
        RandomSource rs = fpc.random();
        WorldGenLevel wgl = fpc.level();
        ModdedOreVeinFeatureConfigurationDetails details = fpc.config().Details;
        ...
        for (BlockPos temp : FeaturePlacementUtils.GetRectangularArea(fpc.origin().offset(-(fs / 2), 0 , -(fs / 2)) , new BlockPos(fs , details.Y_Scale.sample(rs) , fs)))
        {
            ...
            for (var t : (density >= rareplacement) ? details.RarePlacementSettings.RareTargetStates : (density > 0d) ? details.TargetStates : List.<SingleTargetBlockState>of())
            {
