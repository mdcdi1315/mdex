...
import com.github.mdcdi1315.mdex.features.config.SimpleFloatingIslandConfigurationDetails;
...
public final class SimpleFloatingIslandFeature
    extends ModdedFeature<ModdedFeatureConfiguration<SimpleFloatingIslandConfigurationDetails>>
{
    public SimpleFloatingIslandFeature(Codec<ModdedFeatureConfiguration<SimpleFloatingIslandConfigurationDetails>> codec) {
        super(codec);
    }

    @Override
    protected boolean PlaceModdedFeature(FeaturePlaceContext<ModdedFeatureConfiguration<SimpleFloatingIslandConfigurationDetails>> fpc)
    {

        WorldGenLevel wgl = fpc.level();
        SimpleFloatingIslandConfigurationDetails details = fpc.config().Details;
        ...
        layer = layer.offset(0 ,details.MaxDistanceFromGround.sample(fpc.random()) , 0);

        var layers = details.Layers;
        var layerssizem1 = layers.size() - 1;

        // Check that we have enough space to generate the feature
        if (!IsConsideredValidPlacement(wgl , layer , layers.size())) { return false; }
        ...
        FeaturePlacementUtils.ForEachInHolderSetUnsafe(details.FeaturesToGenerateOnTop , fpc , layer , layers.get(layerssizem1).Size , SimpleFloatingIslandFeature::RunTopFeature);

        return true;
    }

    private static void RunTopFeature(PlacedFeature p , FeaturePlaceContext<ModdedFeatureConfiguration<SimpleFloatingIslandConfigurationDetails>> cfg , BlockPos layer , byte lastlayersize)
    {
        var rd = cfg.random();
