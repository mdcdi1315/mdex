    ...
    extends LootPoolEntryContainer
{
    ...
    public MDEXLootTableBaseContainer(List<LootItemCondition> conditions, int weight, int quality)
    {
        super(conditions);
        this.weight = weight;
    ...
    protected static <T extends MDEXLootTableBaseContainer> Products.P3<RecordCodecBuilder.Mu<T>, List<LootItemCondition>, Integer, Integer> CommonBaseEntryFields(RecordCodecBuilder.Instance<T> instance)
    {
        return instance.group(
                new ListCodec<>(LootItemCondition.DIRECT_CODEC).optionalFieldOf("conditions", List.of()).forGetter((T i) -> i.conditions),
                CodecUtils.ZERO_OR_POSITIVE_INTEGER.optionalFieldOf("weight", DEFAULT_WEIGHT).forGetter((T i) -> i.weight),
