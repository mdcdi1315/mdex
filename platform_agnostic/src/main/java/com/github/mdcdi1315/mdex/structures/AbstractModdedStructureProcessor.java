        ...
        if ((state & STATE_MODLIST_DETERMINED) == 0 && InternalDetermineIfModListIsValid())
        {
            try {
                if (MDEXModInstance.LoggingFlags.StructureProcessors()) {
                    MDEXModInstance.LOGGER.info("Compiling structure processor DCO of type {} with hash code {}." , getClass().getName() , hashCode());
                }
                CompileData();
            } catch (Exception e) {
                MDEXModInstance.LOGGER.warn("Reporting exception during COMPILE stage for modded structure processor of type {}: \n{}" , getClass().getName() , e);
                MarkAsInvalid();
                return false;
            }
            if (MDEXModInstance.LoggingFlags.StructureProcessors()) {
                MDEXModInstance.LOGGER.info("Structure processor DCO of type {} with hash code {} was successfully compiled!" , getClass().getName() , hashCode());
            }
            return true;
        }
