    ...
    implements Compilable
{
    ...
    {
        try {
            if (MDEXModInstance.LoggingFlags.StructureProcessors())
            {
                MDEXModInstance.LOGGER.info("Destroying invalid structure class instance named as {}" , getClass().getName());
            }
    ...
    public final void Compile()
    {
        ...
        try {
            if (MDEXModInstance.LoggingFlags.StructureProcessors())
            {
                MDEXModInstance.LOGGER.info("Attempting to compile structure DCO {} with hash code {}." , getClass().getName() , hashCode());
            }
            for (var m : ModIds)
            {
                if (!BaseModsLib.IsModLoaded(m)) {
            ...
            if ((state & STATE_IS_INVALID) == 0) // May found errors during compilation
            {
                ...
                if (MDEXModInstance.LoggingFlags.StructureProcessors()) {
                    MDEXModInstance.LOGGER.info("Structure DCO with hash code {} succeeded with compilation." , hashCode());
                }
            }
    ...
    public final void SetInstanceAsInvalid()
    {
        ...
        if (MDEXModInstance.LoggingFlags.StructureProcessors()) {
            ...
            MDEXModInstance.LOGGER.info("IsInvalid was run for structure configuration object {}. Stack Trace: \n{}" , getClass().getName() , sb);
        } else {
